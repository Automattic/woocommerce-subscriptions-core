...
class WCS_PayPal_Standard_Switcher {
	...
	public static function maybe_cancel_paypal_after_switch( $order_id, $old_status, $new_status ) {
		...
		$order = wc_get_order( $order_id );
		$order_completed = in_array( $new_status, array( apply_filters( 'woocommerce_payment_complete_order_status', WCS_Order_Status::PROCESSING, $order_id, $order ), WCS_Order_Status::PROCESSING, WCS_Order_Status::COMPLETED ), true ) && in_array( $old_status, apply_filters( 'woocommerce_valid_order_statuses_for_payment', array( WCS_Order_Status::PENDING, WCS_Order_Status::ON_HOLD, WCS_Order_Status::FAILED ), $order ), true );
