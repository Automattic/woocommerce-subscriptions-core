...
class WCS_PayPal_Standard_IPN_Handler extends WC_Gateway_Paypal_IPN_Handler {
	...
	protected function process_ipn_request( $transaction_details ) {
		...
		if ( isset( $transaction_details['txn_id'] ) ) {
			...
			$handled_transactions = $subscription->get_meta( '_paypal_ipn_tracking_ids' );
		...
		if ( in_array( $transaction_details['txn_type'], array( 'subscr_signup', 'subscr_payment' ) ) && false !== strpos( $transaction_details['invoice'], '-wcsfrp-' ) ) {
			...
			if ( wcs_get_objects_property( $transaction_order, 'id' ) != $subscription->get_meta( '_paypal_failed_sign_up_recorded' ) ) {
		...
		switch ( $transaction_details['txn_type'] ) {
			case 'subscr_signup':
				...
				if ( $is_payment_change ) {
					...
					if ( 'paypal' == $subscription->get_meta( '_old_payment_method' ) ) {
						self::cancel_subscription( $subscription, $subscription->get_meta( '_old_paypal_subscriber_id' ) );
					}
			...
			case 'subscr_payment':
				...
				if ( 'completed' == strtolower( $transaction_details['payment_status'] ) ) {
					...
					} elseif ( $subscription->get_payment_count() == 1 && '' !== WCS_PayPal::get_option( 'identity_token' ) && 'true' != $subscription->get_meta( '_paypal_first_ipn_ignored_for_pdt' ) && false === $is_renewal_sign_up_after_failure ) {
