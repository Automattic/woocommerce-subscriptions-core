...
class WCS_PayPal {
	...
	public static function process_subscription_payment_response( $order, $response ) {
		...
		if ( $response->has_api_error() ) {
			...
			$order->update_status( WCS_Order_Status::FAILED, sprintf( __( 'PayPal API error: (%1$d) %2$s', 'woocommerce-subscriptions' ), $response->get_api_error_code(), $error_message ) );
			...
			$order_status = apply_filters( 'wcs_paypal_held_payment_order_status', WCS_Order_Status::ON_HOLD, $order, $response );
			...
			// translators: placeholder is PayPal transaction status message
			$order->update_status( WCS_Order_Status::FAILED, sprintf( __( 'PayPal payment declined: %s', 'woocommerce-subscriptions' ), $response->get_status_message() ) );
