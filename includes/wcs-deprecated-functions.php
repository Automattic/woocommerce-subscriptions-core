...
function wcs_get_subscription_in_deprecated_structure( WC_Subscription $subscription ) {
	...
	if ( ! empty( $item ) ) {
		...
		$deprecated_subscription_object = array(
			...
			'length'             => wcs_estimate_periods_between( $subscription->get_time( 'start' ), $subscription->get_time( 'end' ) + 120, $subscription->billing_period, 'floor' ), // Since subscriptions no longer have a length, we need to calculate the length given the start and end dates and the period.
