...
class WCS_Cart_Resubscribe extends WCS_Cart_Renewal {
	...
	public function maybe_setup_cart() {
		...
		if ( isset( $_GET['resubscribe'] ) && isset( $_GET['_wpnonce'] ) ) {
			...
			if ( wcs_get_objects_property( $order, 'order_key' ) === $order_key && $order->has_status( array( WCS_Order_Status::PENDING, WCS_Order_Status::FAILED ) ) && wcs_order_contains_resubscribe( $order ) ) {
