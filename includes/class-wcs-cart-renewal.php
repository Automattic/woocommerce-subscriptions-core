...
class WCS_Cart_Renewal {
	...
	public function maybe_setup_cart() {
		...
		if ( isset( $_GET['pay_for_order'] ) && isset( $_GET['key'] ) && isset( $wp->query_vars['order-pay'] ) ) {
			...
			if ( wcs_get_objects_property( $order, 'order_key' ) === $order_key && $order->has_status( array( WCS_Order_Status::PENDING, WCS_Order_Status::FAILED ) ) && wcs_order_contains_renewal( $order ) ) {
				...
				foreach ( $subscriptions as $subscription ) {
					...
					if ( empty( $subscription ) || ! $subscription->has_status( array( WC_Subscription::STATUS_ON_HOLD, WC_Subscription::STATUS_PENDING ) ) ) {
	...
	*/
	...
	public function maybe_preserve_order_status( $order_status ) {
		...
		if ( null !== WC()->session && WCS_Order_Status::FAILED !== $order_status ) {
			...
			if ( $order && wcs_order_contains_renewal( $order ) && $order->has_status( WCS_Order_Status::FAILED ) ) {
				$order_status = WCS_Order_Status::FAILED;
			}
