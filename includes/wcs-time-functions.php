...
/**
 * Find the next occurring date/day from a given timestamp.
 *
 * Eg get the timestamp for the next March 3rd or first Monday after a given date. Handy
 * for calculating synchronisation dates or next payment dates where a large amount of time
 * has past since the last payment.
 *
 * @param  string $period a billing period: week, month or year.
 * @param  mixed $day_data information about to day/date - the format depends on the period passed:
 * $period week - string|int the full name of the weekday (Monday) or a numerical representation of the day:
 *     - Monday    (1)   - Friday   (5)
 *     - Tuesday   (2)   - Saturday (6)
 *     - Wednesday (3)   - Sunday   (7)
 *     - Thursday  (4)
 * $period month - int a numerical representation of the day of the month (1 - 31)
 * $period year  - array an array containing the day and month eg array( 'day' => 1,'month' => 3 ) (1st of March),
 * @param string $from_date The date to calculate the next occurrence from.
 * @return string
 */
function wcs_get_next_occuring_day( $period, $day_data, $from_timestamp ) {
	$timestamp = 0;
	...
	if ( 'week' == $period ) {
		...
		$weekday = ( is_numeric( $day_data ) ) ? $weekdays[ $day_data ] : $day_data;

		// strtotime() will figure out if the day is in the future or today (see: https://gist.github.com/thenbrent/9698083)
		$timestamp = wcs_strtotime_dark_knight( $weekday, $from_timestamp );
		...
		if ( $day_data > 27 ) { // we actually want the last day of the month

			$day_data = gmdate( 't', $from_timestamp );
			$month    = gmdate( 'F', $from_timestamp );

		} elseif ( gmdate( 'j', $from_timestamp ) > $day_data ) { // today is later than specified day in the from date, we need the next month
		...
		$timestamp = wcs_strtotime_dark_knight( "{$day_data} {$month}", $from_timestamp );

	} elseif ( 'year' == $period ) {

		// We can't use $wp_locale here because it is translated
		switch ( $day_data['month'] ) {
			case 1 :
				$month = 'January';
				break;
			case 2 :
		...
		$timestamp = wcs_strtotime_dark_knight( "{$day_data['day']} {$month}", $from_timestamp );
	}
