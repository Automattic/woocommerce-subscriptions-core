...
class WC_API_Subscriptions extends WC_API_Orders {
	...
	public function create_subscription( $data ) {
		...
		// create subscription - all the checks and defaults are done within that function TODO: check the previous statement is true
		$subscription = wcs_create_subscription( $data );

		if ( is_wp_error( $subscription ) ) {
			return new WP_Error( 'wcs_api_cannot_create_subscription', sprintf( __( 'Cannot create subscription: %s', 'woocommerce-subscriptions' ), implode( ', ', $subscription->get_error_messages() ) ), 400 );
		}

		$this->set_address_fields( $subscription, $data );

		// add line items, tax, shipping class and the lot
		$lines = array(
		...
		foreach ( $lines  as $line_type => $line ) {
			...
			if ( isset( $data[$line] ) && is_array( $data[$line] ) ) {
				...
				$set_item = "set_line{$line_type}";

				foreach ( $data[$line] as $item ) {
					// check if the line item is a subscription product
					$this->$set_item( $subscription, $item, 'create' );
				}
			}
		}
		...
		if ( isset( $data['payment_details'] ) && is_array( $data['payment_details'] ) ) {
			...
			if ( empty( $data['payment_details']['method_id'] ) || empty( $data['payment_details']['method_title'] ) ) {
				return new WP_Error( 'wcs_api_invalid_payment_details', __( 'Recurring payment method ID and title are required', 'woocommerce' ), array( 'status' => 400 ) );
			}

			update_post_meta( $subscription->id, '_payment_method', $data['payment_details']['method_id'] );
			update_post_meta( $subscription->id, '_recurring_payment_method', $data['payment_details']['method_id'] );
			update_post_meta( $subscription->id, '_payment_method_title', $data['payment_details']['method_title'] );
			update_post_meta( $subscription->id, '_recurring_payment_method_title', $data['payment_details']['method_title'] );
			...
			if ( isset( $data['currency'] ) ) {
				...
				if ( ! array_key_exists( $data['currency'], get_woocommerce_currencies() ) ) {
					return new WP_Error( 'woocommerce_invalid_order_currency', __( 'Provided order currency is invalid', 'woocommerce-subscription' ), array( 'status' => 400 ) );
				}
			}
		...
		do_action( 'wcs_api_subscription_created', $subcription_id, $this );

		return array( 'creating_subscription', 'new sub, get on it!' );
	}
