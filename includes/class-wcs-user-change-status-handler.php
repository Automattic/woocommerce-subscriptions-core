...
class WCS_User_Change_Status_Handler {
		...
		switch ( $new_status ) {
			case 'active' :
				if ( ! $subscription->needs_payment() ) {
					...
					$subscription->add_order_note( sprintf( _x( 'Subscription reactivated by the subscriber from their account page.', 'order note left on subscription after user action', 'woocommerce-subscriptions' ) ) );
					WC_Subscriptions::add_notice( sprintf( __( 'Your subscription has been reactivated.', 'Notice displayed to user confirming their action.', 'woocommerce-subscriptions' ) ), 'success' );
					$changed = true;
				} else {
			...
			case 'on-hold' :
				if ( wcs_can_user_put_subscription_on_hold( $subscription ) ) {
					$subscription->update_status( $new_status );
					$subscription->add_order_note( sprintf( _x( 'Subscription put on hold by the subscriber from their account page.', 'order note left on subscription after user action', 'woocommerce-subscriptions' ) ) );
					WC_Subscriptions::add_notice( sprintf( __( 'Your subscription has been put on hold.', 'Notice displayed to user confirming their action.', 'woocommerce-subscriptions' ) ), 'success' );
					$changed = true;
				} else {
			...
			case 'cancelled' :
				...
				$subscription->add_order_note( sprintf( _x( 'Subscription cancelled by the subscriber from their account page.', 'order note left on subscription after user action', 'woocommerce-subscriptions' ) ) );
				WC_Subscriptions::add_notice( sprintf( __( 'Your subscription has been cancelled.', 'Notice displayed to user confirming their action.', 'woocommerce-subscriptions' ) ), 'success' );
				$changed = true;
				break;
		}
	...
	public static function validate_request( $user_id, $subscription, $new_status, $wpnonce = '' ) {
		...
		if ( ! wcs_is_subscription( $subscription ) ) {
			...
			// Needed because this gives me translated versions of status texts
			$stati = wcs_get_subscription_statuses();
			$new_status_key = ( 'wc-' === substr( $new_status, 0, 3 ) ) ? $new_status : 'wc-' . $new_status;;

			// translators: placeholder is subscription's new status, translated
			WC_Subscriptions::add_notice( sprintf( __( 'That subscription can not be changed to %s. Please contact us if you need assistance.', 'woocommerce-subscriptions' ), $stati[ $new_status_key ] ), 'error' );
			return false;
		}
