		...
		<thead>
			<tr>
				...
				<th>
					...
					<?php echo wp_kses( wc_help_tip( __( 'The status of the automatic payment retry: pending means the retry will be processed in the future, failed means the payment was not successful when retried and completed means the payment succeeded when retried.', 'woocommerce-subscriptions' ) ), array( 'span' => array( 'class' => array(), 'data-tip' => array() ) ) ); ?>
				</th>
				<th>
					<?php esc_html_e( 'Status of Order', 'woocommerce-subscriptions' ); ?>
					<?php echo wp_kses( wc_help_tip( __( 'The status applied to the order for the time between when the renewal payment failed or last retry occurred and when this retry was processed.', 'woocommerce-subscriptions' ) ), array( 'span' => array( 'class' => array(), 'data-tip' => array() ) ) ); ?>
				</th>
				<th>
					<?php esc_html_e( 'Status of Subscription', 'woocommerce-subscriptions' ); ?>
					<?php echo wp_kses( wc_help_tip( __( 'The status applied to the subscription for the time between when the renewal payment failed or last retry occurred and when this retry was processed.', 'woocommerce-subscriptions' ) ), array( 'span' => array( 'class' => array(), 'data-tip' => array() ) ) ); ?>
				</th>
				<th>
					<?php esc_html_e( 'Email', 'woocommerce-subscriptions' ); ?>
					<?php echo wp_kses( wc_help_tip( __( 'The email sent to the customer when the renewal payment or payment retry failed to notify them that the payment would be retried.', 'woocommerce-subscriptions' ) ), array( 'span' => array( 'class' => array(), 'data-tip' => array() ) ) ); ?>
				</th>
			</tr>
		</thead>
		<tbody>
