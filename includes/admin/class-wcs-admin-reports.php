...
class WCS_Admin_Reports {
	...
	public static function initialize_reports( $reports ) {
		...
		$reports['subscriptions'] = array(
				...
				'reports' => array(
					...
					'subscription_by_plan' => array(
						'title'       => __( 'Subscriptions by Plan', 'woocommerce-subscriptions' ),
						'description' => '',
						'hide_title'  => true,
						'callback'    => array( 'WC_Admin_Reports', 'get_report' ),
					),
	...
	public static function initialize_reports_path( $report_path, $name, $class ) {
		...
		if ( 'WC_Report_subscription_events_by_date' == $class || 'WC_Report_upcoming_recurring_revenue' == $class ||  'WC_Report_subscription_by_plan' == $class ) {
