...
class WCS_Upgrade_2_0 {
	...
	private static function migrate_dates( $new_subscription, $old_subscription ) {
		...
		$wpdb->update( $wpdb->posts, array( 'post_status' => 'trash' ), array( 'post_type' => ActionScheduler_wpPostStore::POST_TYPE, 'post_content' => wp_json_encode( $old_hook_args ) ), array( '%s', '%s' ) );
		...
		try {
			...
			WCS_Upgrade_Logger::add( sprintf( 'For subscription %d: updated dates = %s', $new_subscription->id, str_replace( array( '{', '}', '"' ), '', wp_json_encode( $dates_to_update ) ) ) );
