...
class WCS_Repair_2_0 {
	...
	public static function repair_period( $subscription, $item_id, $item_meta ) {
		...
		$last_renewal_date        = get_date_from_gmt( wcs_get_objects_property( $last_renewal_order, 'date' ) );
		$last_renewal_timestamp   = wcs_date_to_time( $last_renewal_date );

		$second_renewal_order     = array_shift( $renewal_orders );
		$second_renewal_date      = get_date_from_gmt( wcs_get_objects_property( $second_renewal_order, 'date' ) );
		...
		if ( ! empty( $renewal_orders ) ) {
			...
			$third_renewal_date = get_date_from_gmt( wcs_get_objects_property( $third_renewal_order, 'date' ) );
	...
	public static function repair_end_date( $subscription, $item_id, $item_meta ) {
		...
		if ( 'expired' == $subscription['status'] && array_key_exists( 'expiry_date', $subscription ) && ! empty( $subscription['expiry_date'] ) ) {
			...
			if ( empty( $last_order ) ) {
				...
				$subscription['end_date'] = wcs_add_time( 5, 'hours', wcs_date_to_time( get_date_from_gmt( wcs_get_objects_property( $last_order, 'date' ) ) ) );
