	...
	if ( empty( $product ) ) {
		...
		switch ( $allow_switching ) {
			...
			case 'variable_grouped' :
				$is_product_switchable = ( $product->is_type( array( 'variable-subscription', 'subscription_variation' ) ) || WC_Subscriptions_Product::get_visible_grouped_parent_product_ids( $product ) ) ? true : false;
				break;
			case 'no' :
			default:
				$is_product_switchable = false;
