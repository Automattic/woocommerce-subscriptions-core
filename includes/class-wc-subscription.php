...
class WC_Subscription extends WC_Order {
	 ...
	 *
	 * @param array|int Either an order item (in the array format returned by self::get_items()) or the ID of an order item.
	 * @param  bool $inc_tax Whether or not to adjust sign up fee if prices inc tax - ensures that the sign up fee paid amount includes the paid tax if inc gst
	 * @return bool
	 * @since 2.0
	 */
	public function get_items_sign_up_fee( $line_item, $inc_tax = false ) {
		...
		if ( empty( $this->order ) ) {
			...
			if ( true === $inc_tax && ! empty( $original_order_item ) && ( 'yes' == $this->prices_include_tax || true === $this->prices_include_tax ) ) {
