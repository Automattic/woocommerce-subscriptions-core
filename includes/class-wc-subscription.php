...
class WC_Subscription extends WC_Order {
	...
	protected function calculate_next_payment_date() {
		...
		if ( $trial_end_time > current_time( 'timestamp', true ) ) {
			...
			$next_payment_timestamp = wcs_add_time( $this->get_billing_interval(), $this->get_billing_period(), $from_timestamp, 'apply site time offset' );
			...
			while ( $next_payment_timestamp < ( current_time( 'timestamp', true ) + 2 * HOUR_IN_SECONDS ) && $i < 3000 ) {
				$next_payment_timestamp = wcs_add_time( $this->get_billing_interval(), $this->get_billing_period(), $next_payment_timestamp, 'apply site time offset' );
				$i += 1;
			}
		}
