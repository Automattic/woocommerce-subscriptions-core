...
class WC_Subscription extends WC_Order {
	...
	public function needs_payment() {
		...
		if ( parent::needs_payment() ) {
			$needs_payment = true;
		} elseif ( $parent_order && ( $parent_order->needs_payment() || $parent_order->has_status( array( WCS_Order_Status::ON_HOLD, WCS_Order_Status::CANCELLED ) ) ) ) {
			...
			if ( $order && ( $order->needs_payment() || $order->has_status( array( WCS_Order_Status::ON_HOLD, WCS_Order_Status::FAILED, WCS_Order_Status::CANCELLED ) ) ) ) {
				$needs_payment = true;
			}
		}
	...
	public function can_be_updated_to( $new_status ) {
		...
		switch ( $new_status ) {
			...
			case self::STATUS_TRASH:
				if ( $this->has_status( wcs_get_subscription_ended_statuses() ) || $this->can_be_updated_to( self::STATUS_CANCELLED ) ) {
					$can_be_updated = true;
				} else {
					$can_be_updated = false;
				}
	...
	public function set_status( $new_status, $note = '', $manual_update = false ) {
		if ( ! $this->object_read && in_array( $new_status, [ 'draft', 'auto-draft' ], true ) ) {
			$new_status = apply_filters( 'woocommerce_default_subscription_status', self::STATUS_PENDING );
		}
	...
	public function get_paid_order_statuses() {
		...
		$custom_status = apply_filters( 'woocommerce_payment_complete_order_status', WCS_Order_Status::COMPLETED, $this->get_id(), $this );
			...
			if ( ! isset( $this->cached_payment_count['completed'][ $order_type ] ) ) {
				...
				foreach ( $this->get_related_orders( 'all', $order_type ) as $related_order ) {
					if ( null !== $related_order->get_date_paid() ) {
						...
						if ( $related_order->has_status( WCS_Order_Status::REFUNDED ) ) {
	...
	public function get_failed_payment_count() {
		...
		foreach ( $this->get_related_orders( 'all', 'renewal' ) as $renewal_order ) {
			if ( $renewal_order->has_status( WCS_Order_Status::FAILED ) ) {
	...
	public function can_date_be_updated( $date_type ) {
		...
		switch ( $date_type ) {
			case 'start':
			case 'date_created':
				if ( $this->has_status( array( 'auto-draft', self::STATUS_PENDING ) ) ) {
			...
			case 'trial_end':
				...
				if ( $this->get_payment_count() < 2 && ! $this->has_status( wcs_get_subscription_ended_statuses() ) && ( $this->has_status( self::STATUS_PENDING ) || $this->payment_method_supports( 'subscription_date_changes' ) ) ) {
					$can_date_be_updated = true;
				} else {
					$can_date_be_updated = false;
				}
	...
	public function is_editable() {
		...
		if ( ! isset( $this->editable ) ) {
			...
			if ( $this->has_status( array( self::STATUS_PENDING, 'draft', 'auto-draft' ) ) ) {
				$this->editable = true;
