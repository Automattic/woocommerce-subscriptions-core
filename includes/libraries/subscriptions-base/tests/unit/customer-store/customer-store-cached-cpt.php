 ...
 */
class WCS_Customer_Store_Cached_CPT_Test extends WCS_Customer_Store_Test_Base {
	...
	public function test_get_users_subscription_ids_none() {
		$this->assertEquals( array(), self::$cache_store->get_users_subscription_ids( $this->customer_id_without_subscriptions ) );
	}
	...
	public function test_get_users_subscription_ids() {
		...
		for( $i = 0; $i < 3; $i++ ) {
			$subscription       = WCS_Helper_Subscription::create_subscription( array( 'customer_id' => $this->customer_id ) );
	...
	public function test_get_users_subscription_ids_from_transient_cache() {
		...
		$fake_subscription_ids = array( 123, 456, 789 );
	...
	public function test_add_subscription_id_to_cache() {
		...
		$last_subscription = WCS_Helper_Subscription::create_subscription( array( 'customer_id' => $this->customer_id ) );
