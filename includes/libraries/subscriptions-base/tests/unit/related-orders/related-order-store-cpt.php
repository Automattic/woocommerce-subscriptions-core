 ...
 */
class WCS_Related_Order_Store_CPT_Test extends WCS_Related_Order_Store_Test_Base {
	...
	public function test_get_related_order_ids_none( $relation_type ) {
		...
		$this->assertEquals( array(), self::$store->get_related_order_ids( $subscription, $relation_type ) );
	}
	...
	public function test_get_related_subscription_ids_many( $relation_types ) {
		...
		foreach ( $relation_types as $relation_type ) {
			$subscription_ids[ $relation_type ] = array();
			for( $i = 0; $i < 3; $i++ ) {
				$subscription = WCS_Helper_Subscription::create_subscription();
