 ...
 */
class WCS_Related_Order_Store_Test extends WCS_Related_Order_Store_Test_Base {
	...
	public function get_instance_class() {
		return 'WCS_Related_Order_Test_Store';
	}
	...
	public function test_check_relation_type_valid( $relation_type ) {
		...
		$check_relation_type_method = $this->get_accessible_protected_method( $mock_related_order_store, 'check_relation_type' );
		$this->assertNull( $check_relation_type_method->invoke( $mock_related_order_store, $relation_type ) );
	}

	/**
	 * Check WCS_Related_Order_Store->check_relation_type() throws an exception
	 * for invalid relation types.
	 *
	 * @dataProvider provider_relation_type
	...
	public function test_check_relation_type_invalid( $relation_type ) {
		...
		$mock_related_order_store   = $this->getMockForAbstractClass( 'WCS_Related_Order_Store' );
		$check_relation_type_method = $this->get_accessible_protected_method( $mock_related_order_store, 'check_relation_type' );
		$check_relation_type_method->invoke( $mock_related_order_store, $invalid_relation_type );
	}

	/**
	 * Get an accessible instance of the WCS_Related_Order_Store->check_relation_type() method.
	 *
	 * @return ReflectionMethod
	 */
	protected function get_check_relation_type_method() {
		$mock_related_order_store   = $this->getMockForAbstractClass( 'WCS_Related_Order_Store' );
		$check_relation_type_method = $this->get_accessible_protected_method( $mock_related_order_store, 'check_relation_type' );
		return $check_relation_type_method;
	}
}
