 ...
 */
class WCS_Related_Order_Store_Cached_CPT_Test extends WCS_Related_Order_Store_Test_Base {
	...
	public function test_get_related_order_ids_none( $relation_type ) {
		...
		$this->assertEquals( array(), self::$cache_store->get_related_order_ids( $subscription, $relation_type ) );
	}
	...
	public function test_get_related_order_ids_from_transient_cache() {
		...
		$fake_order_ids = array( 123, 456, 789 );
		...
		$this->assertEquals( array(), self::$cache_store->get_related_order_ids( $subscription, 'switch' ) );
		$this->assertEquals( array(), self::$cache_store->get_related_order_ids( $subscription, 'resubscribe' ) );
	}
	...
	public function test_delete_relation( $relation_type ) {
		...
		foreach( array( $persistent_relation_type, $relation_type ) as $type ) {
			foreach ( array( $subscription_one, $subscription_two ) as $subscription ) {
	...
	public function test_set_empty_renewal_order_cache() {
		...
		$subscription = WCS_Helper_Subscription::create_subscription();
		$this->assertEquals( array(), $this->get_cache_from_source( $subscription, 'renewal' ) );
	}
	...
	public function test_add_related_order_cache_props() {
		...
		$this->assertEquals( $expected, self::$cache_store->add_related_order_cache_props( array(), new WCS_Subscription_Data_Store_CPT() ) );
	}
	...
	public function test_add_related_order_cache_props_ignored() {
		$this->assertEquals( array(), self::$cache_store->add_related_order_cache_props( array(), new StdClass() ) );
	}
