...
class WCS_Functions_Unit_Tests extends WCS_Unit_Test_Case {
	...
	public function test_wcs_get_date_meta_key_filterable() {
		add_filter( 'woocommerce_subscription_date_meta_key_prefix', array(
			$this,
			'_filter_wcs_get_date_meta_key'
		), 10, 2 );

		$this->assertEquals( '_r2d2_foo', wcs_get_date_meta_key( 'foo' ) );

		remove_filter( 'woocommerce_subscription_date_meta_key_prefix', array(
			$this,
			'_filter_wcs_get_date_meta_key'
		), 10 );
	}
	...
	public function test_1_wcs_get_subscriptions_for_product() {
		...
		$subscription1 = WCS_Helper_Subscription::create_subscription( array( 'status' => 'pending', 'start_date' => '2015-07-14 00:00:00', 'customer_id' => $user_id1 ) );
	...
	public function test_4_wcs_get_subscriptions_for_product() {
		...
		$subscriptions = wcs_get_subscriptions_for_product( $product_id1, 'ids' , array( 'subscription_status' => 'expired', 'limit' => 1 )  );
		...
		$subscriptions = wcs_get_subscriptions_for_product( array( $product_id1, $product_id2 ), 'ids' , array( 'subscription_status' => array( 'expired', 'cancelled' ), 'limit' => 2 ) );
		 ...
		 * Test the offset arg.
		 */
		$subscriptions = wcs_get_subscriptions_for_product( $product_id1, 'ids', array( 'limit' => 2, 'offset' => 1 ) );
		...
		$subscriptions = wcs_get_subscriptions_for_product( $product_id1, 'ids', array( 'subscription_status' => 'active', 'limit' => 10, 'offset' => 1 ) );

		$this->assertInternalType( 'array', $subscriptions );
		$this->assertEquals( 0, count( $subscriptions ) );
		unset( $subscriptions );

		$subscriptions = wcs_get_subscriptions_for_product( array( $product_id1, $product_id2 ), 'ids', array( 'limit' => 10, 'offset' => 3 ) );
	...
	public function test_5_wcs_get_subscriptions_for_product() {
		...
		$subscriptions = wcs_get_subscriptions_for_product( array_merge( $variations1, $variations2 ), 'ids' , array( 'subscription_status' => array( 'on-hold', 'active'), 'limit' => 2 ) );
	...
	public function test_6_wcs_get_subscriptions_for_product() {
		...
		$subscriptions = wcs_get_subscriptions_for_product( $variation_2->get_id(), 'ids', array( 'limit' => 10, 'offset' => 1 ) );
