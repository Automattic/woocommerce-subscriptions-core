 ...
 */
class WCS_Post_Meta_Cache_Manager_Test extends WCS_Unit_Test_Case {
	...
	public function test_meta_added( $expected_to_run, $post_id, $meta_key, $meta_value ) {
		...
		$this->expected_args = $expected_args = array(
			'add',
			$post_id,
			$meta_key,
			$meta_value,
	...
	public function check_callback_args() {
		...
		if ( count( $this->expected_args ) > 1 ) {
			...
			if ( is_array( $this->expected_args[2] ) ) {
				$this->assertTrue( in_array( $args[2], $this->expected_args[2] ) );
			} else {
