...
class WCS_Limiter {
	...
	protected static function order_awaiting_payment_for_product( $product_id ) {
		...
		if ( is_object( $order ) && $order->has_status( array( WCS_Order_Status::PENDING, WCS_Order_Status::FAILED ) ) ) {
				...
				if ( $item['product_id'] == $product_id || $item['variation_id'] == $product_id ) {
					$subscriptions = wcs_get_subscriptions(
						array(
							...
							'subscription_status' => WC_Subscription::ACTIVE_OR_WAITING_PAYMENT_STATUSES,
						)
					);
