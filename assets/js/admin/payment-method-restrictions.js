jQuery(function ($) {
   ...
   */
  $(document.body).on("wc_remove_error_tip", function (
    e,
    element,
    removed_error_type
  ) {
    ...
    if (
      "subscription" === product_type &&
      !$(element).is("#_subscription_price") &&
      !$(element).is("#_sale_price")
    ) {
      return;
    }
    ...
    if (
      "variable-subscription" === product_type &&
      !$(element).hasClass("wc_input_subscription_price") &&
      !$(element).is(".wc_input_price[name^=variable_sale_price]")
    ) {
      return;
    }

    // Reformat the product price - remove the decimal place separator and remove excess decimal places.
    var price = accounting.unformat(
      $(element).val(),
      wcs_gateway_restrictions.decimal_point_separator
    );
    price = accounting.formatNumber(
      price,
      wcs_gateway_restrictions.number_of_decimal_places
    );

    if (0 >= price) {
      $(document.body).triggerHandler("wc_subscriptions_add_error_tip", [
        element,
        type_error,
      ]);
    } else {
   ...
   */
  $(document.body).on(
    "change wc_subscriptions_validate_zero_recurring_price",
    "#_subscription_price, #_sale_price, .wc_input_subscription_price, .wc_input_price[name^=variable_sale_price]",
    function () {
      var product_type = $("#product-type").val();
  ...
  $(document.body).on("change", "#product-type", function () {
    ...
    $("#_sale_price, .wc_input_price[name^=variable_sale_price]").each(
      function () {
   ...
   */
  $(document.body).on("wc_subscriptions_add_error_tip", function (
    e,
    element,
    error_type
  ) {
