...
class WC_Subscriptions_Test extends WP_UnitTestCase {
	...
	public function test_can_be_updated_to_pending() {
		...
		foreach ( $this->subscriptions as $status => $subscription ) {
			...
			$actual_result   = $subscription->can_be_updated_to( WC_Subscription::STATUS_PENDING );
	...
	public function test_update_status_exception_thrown_one() {
		...
		$this->subscriptions['active']->update_status( WC_Subscription::STATUS_PENDING );
	}
	...
	public function test_needs_payment_renewal_orders( $status ) {
		...
		if ( WC_Subscription::STATUS_PENDING === $status ) {
		...
		$renewal_order->update_status( WCS_Order_Status::PROCESSING ); // update status also calls save() in WC 3.0+
	...
	public function test_draft_subscription_statuses() {
		...
		$this->assertEquals( WC_Subscription::STATUS_PENDING, wcs_get_subscription( $subscription->get_id() )->get_status() );
	...
	public function test_get_paid_order_statuses() {
		...
		array_push( $expected_result, WC_Subscription::STATUS_PENDING, 'wc-pending' );
		$this->assertEquals( $expected_result, $subscription->get_paid_order_statuses() );
	}

	public function custom_paid_order_status_test_1( $subscription ) {
		return WC_Subscription::STATUS_PENDING;
	}
	...
	public function test_get_last_order() {
		...
		$order->update_status( WCS_Order_Status::FAILED );
		$order->save();
		$this->assertFalse( $subscription->get_last_order( 'ids', array( 'parent', 'renewal' ), array( WCS_Order_Status::FAILED ) ) );
	...
	public function test_payment_failed() {
		...
		$this->assertNotEquals( WCS_Order_Status::FAILED, $order->get_status() );
		$subscription->payment_failed();
		$order = wc_get_order( $order_id ); // With WC 3.0, we need to reinit the order to make sure we have the correct status.

		$this->assertEquals( WCS_Order_Status::FAILED, $order->get_status() );
		$this->assertEquals( WC_Subscription::STATUS_ON_HOLD, $subscription->get_status() );
		...
		$this->assertNotEquals( WCS_Order_Status::FAILED, $renewal_order->get_status() );
